...
public class ConnectionAcceptor<T> {
	 ...
	 *             connection
	 */
	public AsyncConnectionHandler<T> accept(AsyncBroadcastProtocolCallback<T> broadcastProtocolCallback) throws IOException {
		...
		if (sChannel != null) {
			...
			AsyncConnectionHandler<T> handler = AsyncConnectionHandler.create(sChannel, _data, key, broadcastProtocolCallback);
