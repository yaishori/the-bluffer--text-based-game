 ...
 */
public class AsyncConnectionHandler<T> {
	...
	private AsyncBroadcastProtocolCallback<T> broadcastProtocolCallback;
	
	public AsyncBroadcastProtocolCallback<T> getBroadcastProtocolCallback() {
		return broadcastProtocolCallback;
	}
	 ...
	 */
	private AsyncConnectionHandler(SocketChannel sChannel,
			                       ReactorData<T> data,
			                       SelectionKey key,
			                       AsyncBroadcastProtocolCallback<T> broadcastProtocolCallback) {
		_sChannel = sChannel;
		_data = data;
		_protocol = _data.getProtocolMaker().create(data.getTbgpData());
	...
	public static <T> AsyncConnectionHandler<T> create(SocketChannel sChannel,
			                                           ReactorData<T> data,
			                                           SelectionKey key,
			                                           AsyncBroadcastProtocolCallback<T> broadcastProtocolCallback) {
		AsyncConnectionHandler<T> h = new AsyncConnectionHandler<T>(sChannel, data, key, broadcastProtocolCallback);
		h.initialize();
		return h;
	}
